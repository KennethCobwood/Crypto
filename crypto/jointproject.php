...
$sql = 'DROP DATABASE covidRegister;';
if (!$conn->query($sql) === TRUE) {
die('Error dropping database: ' . $conn->error);
}
}
$sql = 'CREATE DATABASE IF NOT EXISTS covidRegister;';
if (!$conn->query($sql) === TRUE) {
die('Error creating database: ' . $conn->error);
}
$sql = 'USE covidRegister;';
if (!$conn->query($sql) === TRUE) {
die('Error using database: ' . $conn->error);
}
$sql = 'CREATE TABLE IF NOT EXISTS covidregistrationform (
...
$sql = "INSERT INTO covidregistrationform (iv, firstname, surname, pps) VALUES ('$iv_hex', '$firstname_hex', '$surname_hex', '$pps_hex' )";
if ($conn->query($sql) === TRUE) {
...
<?php
$sql = "SELECT id, iv, firstname, surname, pps FROM covidregistrationform";
...
$unencrypted_firstname = openssl_decrypt($firstname, $surname, $pps, $cipher, $key, OPENSSL_RAW_DATA, $iv);
